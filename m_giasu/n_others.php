<div style="font-size:16px;color:#666666;font-family:Arial, Helvetica, sans-serif">Home&raquo;Liên kết đào tạo và hợp tác</div>	<?php
	  if(!isset($_GET["vt"])) $vt=0;
	  else $vt=intval($_GET["vt"]);
	  
	  $sql="select id_tin,DATE_FORMAT(capnhat,'Cập nhật lúc %H:%i ngày %d/%m/%Y') as hs,DATE_FORMAT(capnhat,'(%d/%m/%Y)') as dt, anhtrichdan,tieude$noi,trichdan$noi from gws_tinbai where  kiemduyet=1 and id_parent=".get_id("hop-tac")." or id_parent=".get_id("dao-tao-dh")." or id_parent=".get_id("dao-tao-kn")." or id_parent=".get_id("dao-tao-nl")." or id_parent=".get_id("dang-ky")."  ";
	...
	?>
	 ...
	 <div class="news">
                    ...
                    <div class="title"> <a style="text-decoration:none;color:#0066FF;font-weight:bold;"href="./?page=lien-ket&code=<?php echo get_key($row["id_parent"]);?>&id=<?php echo $row["id_tin"]?>"><?php echo $row["tieude$noi"];?></a></div>
