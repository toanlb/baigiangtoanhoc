...
function lay_duongdan($id)
{
  ...
  {  
    ...
    while($id!=0)
    {	    
        $sql="select id_parent,tukhoa from gws_ykien gws_danhmuc where id=$id";
...
<?php
  if(isset($_GET["vector"]))
  {
	...
	{
	  if($id_parent!=0) $sql="select id_lienhe,capnhat from gws_ykien where id_parent=$id_parent order by capnhat asc";
	  else $sql="select id_lienhe,capnhat from gws_ykien order by capnhat asc";
  ...
  if(isset($_GET["id_lienhe"]))
  {
	...
	$sql="delete from gws_ykien where id_lienhe=$id_lienhe";
	mysql_query($sql);
  }
?>
<html>
...
function doXuly(ck)
{
  ...
  ifXuly.location="quantriykien/xuly.php?id="+ck.name+"&trangthai="+tt;
}
</script>
<body>
...
<table width="99%" border="0" align="center" cellpadding="4" cellspacing="0">
  <tr>
    <td colspan="7"><font size=2><b>Form ý kiến- Danh sách ý kiến </b></font></td>
  </tr>
  <tr class="gridTitle">
...
<?php
$sql="select *, DATE_FORMAT(capnhat,'%H:%i ngày %d-%m-%Y') as datetime from gws_ykien where trim(tenkhach)<>''";
...
<?php $i=$vt;if(mysql_num_rows($result)!=0) while($row=mysql_fetch_array($result)){?>
  <tr class="gridRow">
	...
	<td class="gridRow" align="center">
	  <a href="javascript:var oW=window.open('quantriykien/view_detail.php?id_lienhe=<?php echo $row["id_lienhe"];?>','','400','300');">xem</a>
