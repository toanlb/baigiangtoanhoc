...
function lay_duongdan($id)
{
  ...
  {  
    ...
    while($id!=0)
    {	    
        $sql="select id_parent,tukhoa from gws_dangky gws_danhmuc where id=$id";
...
<?php
  if(isset($_GET["vector"]))
  {
	...
	{
	  if($id_parent!=0) $sql="select id_dangky,capnhat from gws_dangky where id_parent=$id_parent order by capnhat asc";
	  else $sql="select id_dangky,capnhat from gws_dangky order by capnhat asc";
	  ...
	  if(mysql_num_rows($resultvt)!=0)
	  {
		...
		$sql="update  set capnhat='$capnhatdau' where id_dangky=$id_cuoi";
		mysql_query($sql);
		$sql="update  set capnhat='$capnhatcuoi' where id_dangky=$id_dau";
		mysql_query($sql);
	  }
	}
  }
  ...
  if(isset($_GET["id_dangky"]))
  {
	...
	$sql="delete from gws_dangky where id_dangky=$id_dangky";
	mysql_query($sql);
  }
?>
<html>
...
<table width="99%" align="center" border="0" cellspacing="0" cellpadding="4">
  <tr>
    ...
    <td align="right"><form name="frmSubmit" action="?id_parent=<?php echo $id_parent;?>&kieunhap=<?php echo $kieunhap;?>" method="post">
      Tìm kiếm
      <input name="hoten" type="text" value="<?php if(isset($_POST["hoten"])) echo $_POST["hoten"];?>" >
...
<table width="99%" border="0" align="center" cellpadding="4" cellspacing="0">
  <tr>
    <td colspan="7"><font size=2><b>Form đăng ký - Danh sách </b></font></td>
  </tr>
  <tr class="gridTitle">
...
<?php
$sql="select *, DATE_FORMAT(capnhat,'%H:%i ngày %d-%m-%Y') as datetime from gws_dangky where trim(hoten)<>''";
...
$sql.=" limit $sl";
$result=mysql_query($sql);
for($bg=1;$bg<$vt;$bg++) {$row=mysql_fetch_array($result);$idluu=$row["id_dangky"];}
?>
...
<?php $i=$vt;if(mysql_num_rows($result)!=0) while($row=mysql_fetch_array($result)){?>
  <tr class="gridRow">
	...
	<td class="gridRow">&nbsp;&nbsp;
	  <?php 
	  if(isset($_GET["vector"])){ if($vector=="xuong"&&intval($id_dau)==intval($row["id_dangky"])) echo "<span class='doiVitri'>".$row["tieude"]."</span>";
	  elseif($vector=="len"&&intval($id_cuoi)==intval($row["id_dangky"])) echo "<span class='doiVitri'>".$row["tieude$lang"]."</span>";
	  else echo $row["tieude$lang"];}else echo $row["hoten"];?></td>
	<td class="gridRow"><a href="mailto:<?php echo $row["mail"];?>"><?php echo $row["mail"];?></a></td>
	
	<td class="gridRow"><?php echo $row["datetime"];?></td>
	<td class="gridRow" align="center"><input type="checkbox" name="<?php echo $row["id_dangky"];?>" <?php if($row["xuly"]==1) echo "checked";?> onClick="doXuly(this)"></td>
	<td class="gridRow" align="center">
	  <a href="javascript:var oW=window.open('quantridangky/view_detail.php?id_dangky=<?php echo $row["id_dangky"];?>','','400','300');">xem</a>
	  <a href="javascript:if(confirm('Xóa <?php echo $row["hoten"];?>?')) window.location='?id_parent=<?php echo $id_parent;?>&kieunhap=<?php echo $kieunhap;?>&id_dangky=<?php echo $row["id_dangky"];?>';">xóa</a></td>
  </tr>
	<?php $i++;$idluu=$row["id_dangky"];}else{?>		  
  <tr>
...
<script language="javascript">frmSubmit.hoten.focus();</script>
</body>
</html>