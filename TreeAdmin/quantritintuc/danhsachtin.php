...
if(isset($_POST["exe_action"]))
{
    ...
    foreach($postArray as $sForm=>$value)
    {
	  ...
	  {
	  	$sql="delete from gws_tinbai where id_tin=$gt";
		mysql_query($sql);
	  }
    }		
}
...
<?php
  if(isset($_GET["vector"]))
  {
	...
	{
	  if($id_parent!=0) $sql="select id_tin,capnhat from gws_tinbai where id_parent=$id_parent order by capnhat desc";
	  else $sql="select id_tin,capnhat from gws_tinbai order by capnhat desc";
	  ...
	  if(mysql_num_rows($resultvt)!=0)
	  {
		...
		$sql="update gws_tinbai set capnhat='$capnhatdau' where id_tin=$id_cuoi";
		mysql_query($sql);
		$sql="update gws_tinbai set capnhat='$capnhatcuoi' where id_tin=$id_dau";
		mysql_query($sql);
	  }
	}
  }
  ...
  {
    $id_tin=$_GET["id_tin"];    
	$sql="delete from gws_tinbai where id_tin=$id_tin";
	mysql_query($sql);
  }
?>
<html>
...
function doKiemduyet(ck)
{
  ...
  ifKiemduyet.location="quantritintuc/kiemduyet.php?id="+ck.name+"&trangthai="+tt;
}
function doTieudiem(ck)
{
  if(ck.checked) tt=1;else tt=0;
  ifKiemduyet.location="quantritintuc/tieudiem.php?id="+ck.name+"&trangthai="+tt;
}
function doTinnong(ck)
{
  if(ck.checked) tt=1;else tt=0;
  ifKiemduyet.location="quantritintuc/tinnong.php?id="+ck.name+"&trangthai="+tt;
}
</script>
<body>
...
<table width="99%" class="mainTb" border="0" align="center" cellpadding="4" cellspacing="1">
  <tr>
    <td colspan="7"><font size=2><b>Quản lý Tin tức - Danh sách tin</b></font></td>
  </tr>
  <tr>  
	...
	<td width="8%" align="center" class="gridTitle">Tin nóng</td>
	<td width="6%" align="center" class="gridTitle">T.điểm</td>
...
<?php
$sql="select * from gws_tinbai where trim(tieude$lang)<>'' and id_parent=$id_parent";
...
<?php $i=$vt;if(mysql_num_rows($result)!=0) while($row=mysql_fetch_array($result)){?>
  <tr>
	...
	<td class="gridRow"><input name="ckdel<?php echo $row["id_tin"];?>" type="checkbox" id="ckdel<?php echo $row["id_tin"];?>" value="<?php echo $row["id_tin"];?>">
	&nbsp;&nbsp;
	  <?php 
	  if(isset($_GET["vector"])){ if($vector=="xuong"&&intval($id_dau)==intval($row["id_tin"])) echo "<span class='doiVitri'>".$row["tieude$lang"]."</span>";
	  elseif($vector=="len"&&intval($id_cuoi)==intval($row["id_tin"])) echo "<span class='doiVitri'>".$row["tieude$lang"]."</span>";
	  else echo $row["tieude$lang"];}else echo $row["tieude$lang"];?>   
	</td>
	<td class="gridRow" align="center"><input type="checkbox" name="<?php echo $row["id_tin"];?>" <?php if($row["tinnong"]==1) echo "checked";?> onClick="doTinnong(this);"></td>
	<td class="gridRow" align="center"><input type="checkbox" name="<?php echo $row["id_tin"];?>" <?php if($row["tieudiem"]==1) echo "checked";?> onClick="doTieudiem(this);"></td>
	<td class="gridRow" align="center"><input type="checkbox" name="<?php echo $row["id_tin"];?>" <?php if($row["kiemduyet"]==1) echo "checked";?> onClick="doKiemduyet(this);"></td>
	...
	<td class="gridRow" align="center">
	  <a href="javascript:window.location='formSua.php?id_parent=<?php echo $id_parent;?>&kieunhap=form_suatin&id_tin=<?php echo $row["id_tin"];?>';"><?php echo $arr_tin["ds_suatin"] ?></a>
	  <a href="javascript:if(confirm('<?php echo $arr_tin["ds_xacnhan_xoa_tin"] ?><?php echo $row["tieude$lang"];?>?')) window.location='?id_parent=<?php echo $id_parent;?>&kieunhap=<?php echo $kieunhap;?>&id_tin=<?php echo $row["id_tin"];?>';"><?php echo $arr_tin["ds_xoatin"] ?></a>
	</td>
  </tr>
	<?php $i++;$idluu=$row["id_tin"];}else{?>		  
  <tr>
	<td colspan="7" height="30" class="gridRow"><?php echo $arr_chung["chung_chuaco_noidung"] ?></td>
  </tr>
	<?php }?>
</form>
  <tr><td height="30" colspan="7" class="gridRow"><a href="javascript:deleteAll();"><strong><font color="#3300FF">Xóa các lựa chọn</font></strong></a></td>
  </tr>
  <tr>
