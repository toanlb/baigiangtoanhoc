	...
	$sql="select id_tin,capnhat,tieude$noi,username, DATE_FORMAT(capnhat,'%d-%m') as dt,trichdan$noi from gws_tinbai where trim(tieude$noi)<>'' and id_parent=".get_id("so-tay-dien-tu")." and kiemduyet=1 ";
		 ...
		 <div class="block-l">
          <div class="mod-l">
            <div class="mod-l-in">
              ...
              <h3><a href="./?page=so-tay-dien-tu&code=<?php echo get_key($row["id_parent"]);?>&id=<?php echo $row["id_tin"]?>"><? echo $row["tieude$noi"]?> </a></h3>
...
//]]>
              ...
              <div class="details">
                <a href="./?page=so-tay-dien-tu&code=<?php echo get_key($row["id_parent"]);?>&id=<?php echo $row["id_tin"]?>" class="hold-img"><img src="<? echo $row["anhtrichdan"]?>" width="245" height="175" alt="img" /></a>
                <div> <a href="./?page=so-tay-dien-tu&code=<?php echo get_key($row["id_parent"]);?>&id=<?php echo $row["id_tin"]?>"> <? echo $row["trichdan$noi"]?> </a></div>
          <!--end .box-title-->
	
              <a href="./?page=so-tay-dien-tu&code=<?php echo get_key($row["id_parent"]);?>&id=<?php echo $row["id_tin"]?>" class="read-more">Chi tiết</a> </div>
            </div>
          </div>
